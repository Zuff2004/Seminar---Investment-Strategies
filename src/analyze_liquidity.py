...
def format_brl(value):
    """
    Formats BRL values for terminal printouts.
    """
...
def create_liquidity_summary(selected_pairs_df):
    """
    Creates compact summary tables for the Excel output.
    """
    if selected_pairs_df.empty:
        return pd.DataFrame()

    summary_items = {
        "total_selected_company_pairs": len(selected_pairs_df),
        "pairs_with_ok_data": int((selected_pairs_df["data_status"] == "OK").sum()),
        "pairs_with_long_history": int((selected_pairs_df["data_availability_bucket"] == "Long history").sum()),
        "suggested_base_liquidity_candidates": int(selected_pairs_df["suggested_base_liquidity_flag"].sum()),
        "suggested_strict_liquidity_candidates": int(selected_pairs_df["suggested_strict_liquidity_flag"].sum()),
        "median_pair_avg_daily_value": selected_pairs_df["pair_avg_daily_value"].median(),
        "median_pair_median_daily_value": selected_pairs_df["pair_median_daily_value"].median(),
        "max_pair_avg_daily_value": selected_pairs_df["pair_avg_daily_value"].max(),
    }
...
def main():
    ...
    liquidity_summary = create_liquidity_summary(selected_pairs_df)
    ...
    with pd.ExcelWriter(OUTPUT_FILE, engine="openpyxl") as writer:
        ...
        liquidity_summary.to_excel(writer, sheet_name="Liquidity_Summary", index=False)
